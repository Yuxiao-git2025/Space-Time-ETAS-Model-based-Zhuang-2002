# Space-Time-ETAS-Model-based-Zhuang-2002
This is a MATLAB reproduction of the `spatial-temporal ETAS model` proposed by Teacher Zhuang in 2002, which is more concise and easier to understand. <br>
---
* It supports `custom` input and output. The code uses `global` variables, which might be one of the reasons for the litte slower running speed. Therefore, please use it with caution.<br>
* The `APP` program has been designed. Please contact the author if needed.<br>
* Reference: <br>
|| Zhuang, J., Ogata, Y., & Vere-Jones, D. (2002). Stochastic Declustering of Space-Time Earthquake Occurrences. Journal of the American Statistical Association, 97, 369–380. https://doi.org/10.1198/016214502760046925 <br>
<img width="948" height="482" alt="Sim6" src="https://github.com/user-attachments/assets/4cfa9b22-be3e-495d-83e0-579959a3f0a8" />
<img width="977" height="496" alt="Sim5" src="https://github.com/user-attachments/assets/d7acbecf-f60b-4957-8b44-d276e201d886" />
<img width="975" height="493" alt="Sim4" src="https://github.com/user-attachments/assets/0e407cd2-62f3-4387-a250-107d56b2e76f" />
<img width="725" height="412" alt="Sim3" src="https://github.com/user-attachments/assets/74c3ea67-052c-4979-94ee-e02360879789" />
<img width="1200" height="500" alt="Main_SpaceModel_17" src="https://github.com/user-attachments/assets/07cb773e-cdfa-4bbd-861b-b8bd343513cb" />

